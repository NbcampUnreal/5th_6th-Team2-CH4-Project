...
void AT2GameModeBase::TryStartCountdown()
{
    if (bCountdownRunning) return;
    if (GetNumPlayers() < RequiredPlayers) return;

    bCountdownRunning = true;
    CountdownLeft = 5;

    UE_LOG(LogTemp, Warning, TEXT("Timer is running"));

    if (AT2GameStateBase* GS = GetGameState<AT2GameStateBase>())
        GS->Countdown = CountdownLeft;

    GetWorldTimerManager().SetTimer(CountdownTimer, this, &ThisClass::TickCountdown, 1.0f, true);
}

void AT2GameModeBase::CancelCountdown()
{
...
void AT2GameModeBase::TickCountdown()
{
    CountdownLeft--;
    ...
    if (CountdownLeft <= 0)
    {
        ...
        GetWorld()->ServerTravel(TEXT("/Game/Library_Pack/Maps/Example1"), true);
    }
}
