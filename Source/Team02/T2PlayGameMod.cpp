...
AT2PlayGameMod::AT2PlayGameMod()
{
    ...
    KillerPawnClass = AT2KillerCharacter::StaticClass();
    SurvivorPawnClass = AT2PlayerCharacter::StaticClass();
}
...
{
    if (bRolesAssigned) return;
    if (GetNumPlayers() < RequiredPlayers) return;

    TArray<AT2PlayerState*> Players;

    for (APlayerState* PS : GameState->PlayerArray)
    {
        if (AT2PlayerState* T2PS = Cast<AT2PlayerState>(PS))
        {
            Players.Add(T2PS);
        }
    }
    ...
    // 첫 명 Killer, 나머지 Survivor
    Players[0]->SetPlayerRole(EPlayerRole::Killer);
    for (int32 i = 1; i < Players.Num(); ++i)
    {
        Players[i]->SetPlayerRole(EPlayerRole::Survivor);
    }
...
UClass* AT2PlayGameMod::GetDefaultPawnClassForController_Implementation(AController* InController)
{
    if (APlayerController* PC = Cast<APlayerController>(InController))
    {
        ...
        {
            if (PS->PlayerRole == EPlayerRole::Killer) return KillerPawnClass;

            if (PS->PlayerRole == EPlayerRole::Survivor) return SurvivorPawnClass;
        }
    }
    return Super::GetDefaultPawnClassForController_Implementation(InController);
}

void AT2PlayGameMod::PostSeamlessTravel()
{
...
void AT2PlayGameMod::OnSurvivorDied()
{
    AT2PlayGameState* GS = GetGameState<AT2PlayGameState>();
    GS->AliveSurvivorCount--;

    if (GS->AliveSurvivorCount <= 0)
    {
        // Killer Win
        UE_LOG(LogTemp, Warning, TEXT("EndMatch();"));
    }
}

void AT2PlayGameMod::OnKeyCollected()
{
    AT2PlayGameState* GS = GetGameState<AT2PlayGameState>();
    GS->CollectedKeys++;

    if (GS->CollectedKeys >= 5)
    {
        // Survivor Win
        UE_LOG(LogTemp, Warning, TEXT("EndMatch();"));
    }
